...
def apply_filters(config: dict, df: pd.DataFrame) -> pd.DataFrame:
    ...
    def get_acceptable_elements_boolean(stoichiometry: dict, acceptable_els: list) -> bool:
    ...
    for name, val in element_filters.items():
        if (str(val) != "None"):  # depending on how yaml is created, val may either be "None" or NoneType
            if name == "filter_by_acceptable_elements":
                df['filter_acceptable_els'] = df.stoichiometry.apply(get_acceptable_elements_boolean, args = (val,))
                df = df[df.filter_acceptable_els]
                df = df.drop(columns = ['filter_acceptable_els'])
            elif name == "filter_by_required_elements":
                df['filter_required_els'] = df.stoichiometry.apply(get_required_elements_boolean, args = (val,))
                df = df[df.filter_required_els]
                df = df.drop(columns = ['filter_required_els'])
                
            elif name == 'filter_by_number_elements':
                df['filter_number_els'] = df.stoichiometry.apply(get_number_elements_boolean, args = (val,))
...
def get_specific_smile_plot(smile: str, df: pd.DataFrame, npz_path: str):
    ...
    if len(df_smile_specific.distribution.tolist()) == 0:
        ...
        slope_overall, intercept_overall, r_overall, p, se = linregress(x_overall,y_overall)
        f, (ax1, ax2, ax3) = plt.subplots(1, len(types)+1, sharey=True)
        ax1.set_title(smile + ' overall')
        ax1.plot([-4,2],[-4,2], 'k-', linewidth=3)
        ax1.plot([-4,2],[-4*slope_overall + intercept_overall,2*slope_overall + intercept_overall], 'k--', linewidth=2)
        ...
        ax1.legend(['y = x', f'y = {slope_overall:1.2f} x + {intercept_overall:1.2f}, R-sq = {r_overall**2:1.2f}'], loc = 'lower right')
        ax1.axis('square');
        ax1.set_xlim([-4,2]);
        ax1.set_ylim([-4,2]);
        ...
        ax2.plot([-4,2],[-4*slope_now + intercept_now,2*slope_now + intercept_now], 'k--', linewidth=2)
        ...
        ax2.legend(['y = x', f'y = {slope_now:1.2f} x + {intercept_now:1.2f}, R-sq = {r_now**2:1.2f}'], loc = 'lower right')
        ax2.axis('square');
        ax2.set_xlim([-4,2]);
        ax2.set_ylim([-4,2]);
        ...
        ax3.plot([-4,2],[-4*slope_now + intercept_now,2*slope_now + intercept_now], 'k--', linewidth=2)
        ...
        ax3.legend(['y = x', f'y = {slope_now:1.2f} x + {intercept_now:1.2f}, R-sq = {r_now**2:1.2f}'], loc = 'lower right')
        ax3.axis('square');
        ax3.set_xlim([-4,2]);
        ax3.set_ylim([-4,2]);
...
def get_general_plot(df: pd.DataFrame, npz_path: str):
    ...
    ax4.plot([-4,2],[-4*slope_now + intercept_now,2*slope_now + intercept_now], 'k--', linewidth=2)
    ...
    ax4.legend(['y = x', f'y = {slope_now:1.2f} x + {intercept_now:1.2f}, R-sq = {r_now**2:1.2f}'], loc = 'lower right')
    ax4.axis('square');
    ax4.set_xlim([-4,2]);
    ax4.set_ylim([-4,2]);
    ...
    ax5.plot([-4,2],[-4*slope_now + intercept_now,2*slope_now + intercept_now], 'k--', linewidth=2)
    ...
    ax5.legend(['y = x', f'y = {slope_now:1.2f} x + {intercept_now:1.2f}, R-sq = {r_now**2:1.2f}'], loc = 'lower right')
    ax5.axis('square');
    ax5.set_xlim([-4,2]);
    ax5.set_ylim([-4,2]);
